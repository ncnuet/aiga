...
def sentence1() -> Expr:
    ...
    sen1_1 = disjoin(A, B)
    sen1_2 = ~A % (~B | C)
    sen1_3 = disjoin(~A, ~B, C)
    return conjoin(sen1_1, sen1_2, sen1_3)
...
def sentence2() -> Expr:
    ...
    sen2_3 = ~(B & ~C) >> A
    sen2_4 = ~D >> C
    return conjoin(sen2_1, sen2_2, sen2_3, sen2_4)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def sentence3() -> Expr:
    ...
    sen3_1 = A % ((B & ~D) | (~B & C))
    sen3_2 = ~(B & C)
    return conjoin(sen3_1, sen3_2, C)
    util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def mapping(problem, agent) -> Generator:
    ...
    for t in range(agent.num_timesteps):
        ...
        for coord in non_outer_wall_coords:
            x, y = coord
            check_wall = PropSymbolExpr(wall_str, x, y)

            if entails(conjoin(KB), check_wall):
                KB.append(check_wall)
                known_map[x][y] = 1
            elif entails(conjoin(KB), ~check_wall):
                KB.append(~check_wall)
                known_map[x][y] = 0
            else:
                known_map[x][y] = -1
